...
B = [list(sys.stdin.readline().rstrip().rstrip()) for _ in range(N)]  # Board
dx = [-1, 1, 0, 0]  # x축 움직임
dy = [0, 0, -1, 1]  # y축 움직임
...
def solve():
    ...
    while queue:  # BFS : queue 기준
        rx, ry, bx, by, depth,route = queue.popleft()2
        if depth > 10:  # 실패 조건
            break
        for i in range(4):  # 4방향 이동 시도
            ...
            if B[nbx][nby] != 'O':  # 실패 조건이 아니면
                if B[nrx][nry] == 'O':  # 성공 조건
                    print(depth,route+direction[i],sep="\n")
                    return
                ...
                if not visited[nrx][nry][nbx][nby]:
                    ...
                    queue.append((nrx, nry, nbx, nby, depth+1,route+direction[i]))
    print(-1)  # 실패 시
