...
while queue:
    ...
    for i in range(4):
        ...
        if 0 <= tx < X and 0 <= ty < Y and board[ty][tx] != 1:
            if horse == 0:
                if visited[ty][tx][0] == 0 or visited[ty][tx][0] >= cnt+1:
                    visited[ty][tx][0] = cnt+1
                    queue.append([ty, tx, cnt+1, horse])
            else:
                if visited[ty][tx][horse] == 0 or visited[ty][tx][horse] >= cnt+1:
    ...
    if horse < K:
        for i in range(8):
            ...
            if 0 <= tx < X and 0 <= ty < Y and board[ty][tx] != 1:
                if visited[ty][tx][horse+1] == 0 or visited[ty][tx][horse+1] >= cnt+1:
