...
def StreeInit(left, right, node):
    ...
    if left == right:
        ...
        mid = (left+right) // 2
        tree[node] = min(StreeInit(left, mid, node*2),
...
def StreeFind(left, right, node, targetLeft, targetRight):
    ...
    # 우리가 찾으려넌 범위 밖에 오른쪽 왼쪽이 둘다 넘어갈 경우
        ...
        # 왼쪽 오른쪽 탐색
        mid = (left+right) // 2
        return min(StreeFind(left, mid, node*2, targetLeft, targetRight), StreeFind(mid+1, right, node*2 + 1, targetLeft, targetRight))
    return sys.maxsize
...
for _ in range(M):
    ...
    print(StreeFind(0, numN-1, 1, a-1, b-1))
